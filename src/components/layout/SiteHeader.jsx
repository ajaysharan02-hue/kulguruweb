...
export function SiteHeader({ brandName = 'Kulgurusp Institute', logo, phone, email }) {
            ...
            <Container className="flex items-center justify-between py-4">
                <Link href="/" className="flex min-w-0 items-center gap-3">
                    {logo ? (
                        ...
                        <img src={logo} alt={brandName} className="md:w-32 w-18" />
                    ) : (
                    ...
                    {items.map(i => {
                        ...
                        return (
                            <Link
                                key={i.href}
                                href={i.href}
                                className={`rounded-xl px-4 py-2 transition ${active ? 'bg-(--primary)/12 text-(--primary)' : 'hover:bg-(--accent)/10 hover:text-(--primary)'}`}>
                                {i.label}
                            </Link>
                        );
                    })}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        aria-expanded={open}
                        onClick={() => setOpen(v => !v)}>
                        {open ? <span className="text-xl leading-none">×</span> : <span className="text-xl leading-none">☰</span>}
                    </button>
                </div>
            </Container>
                            ...
                            {items.map(i => {
                                ...
                                return (
                                    <Link
                                        key={i.href}
                                        href={i.href}
                                        className={`rounded-2xl px-4 py-3 text-sm font-semibold transition ${active ? 'bg-(--primary)/12 text-(--primary)' : 'text-slate-700 hover:bg-(--accent)/10'}`}
